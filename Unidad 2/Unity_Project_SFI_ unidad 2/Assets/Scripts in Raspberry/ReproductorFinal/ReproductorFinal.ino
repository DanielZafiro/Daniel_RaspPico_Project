        ...
        switch (command) {
            case 'V': // Comando para cambiar el volumen
                if (Serial.available() >= 2) { // Verificar si hay suficientes datos disponibles
                    ...
                    if (newVolume >= 0 && newVolume <= 100) { // Verificar si el nuevo volumen está dentro del rango permitido
                        volumen = newVolume; // Actualizar el volumen
                        Serial.println("Volumen cambiado a: " + String(volumen));
                    } else {
                        Serial.println("Error: Volumen fuera de rango (0-100).");
                    }
                }
                break;
...
void task() {
    ...
    if (duracionCancion > 0) {
        // Implementar la lógica para cambiar la duración de la canción en tiempo real
        // Por ejemplo, podrías restar un segundo de la duración de la canción en cada iteración de esta función
